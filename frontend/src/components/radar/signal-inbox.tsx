...
const RadarCanvas = ({ signals, isAwaiting }: { signals: Signal[]; isAwaiting: boolean }) => {
    ...
    // Stable blip positions for signals
    ...
    useEffect(() => {
        ...
        // Always add HITL blip when awaiting
        if (isAwaiting) {
    ...
    useEffect(() => {
        ...
        const draw = () => {
            ...
            cGrad.addColorStop(0, "rgba(57,255,20,0.9)");
            cGrad.addColorStop(1, "rgba(57,255,20,0)");
            ctx.beginPath();
            ...
            blipPositions.current.forEach(({ x, y, signal }) => {
                ...
                // Check if sweep just passed the blip — brightened effect
                ...
                if (signal.urgency === "CRITICAL") {
                    ...
                    ctx.strokeStyle = `rgba(255,45,85,${pulseAlpha * fadeAlpha})`;
                    ctx.lineWidth = 1;
                    ctx.stroke();
                }
            });
...
export const SignalInbox = ({ onOpenReview }: Props) => {
    ...
    // Dynamic signals driven by workflow state
    const signals: Signal[] = [];
    if (isAwaiting) {
        signals.push({
            id: "hitl-pending",
            title: "HITL AUTHORIZATION",
            source: "AI ORCHESTRA",
    ...
    return (
        <div className="w-full h-full flex flex-col gap-3">
            ...
            {/* ── HITL Authorization Gate / Signal Cards ── */}
            ...
            >
                ...
                <div className="flex items-center justify-between px-4 py-2 border-b border-white/5 shrink-0">
                    <div className="flex items-center gap-2">
                        <Crosshair size={12} className={isAwaiting ? "text-cyber-amber animate-spin" : "text-white/25"} style={isAwaiting ? { animationDuration: "3s" } : {}} />
                    ...
                    <AnimatePresence mode="wait">
                        {isAwaiting ? (
                            <motion.div key="awaiting" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="flex flex-col gap-2">
                                {missionMessage && (
                                    <p className="font-mono text-[9px] text-cyber-amber/70 truncate">
                                        ↳ {missionMessage.slice(0, 55)}...
                                    </p>
                                )}
                                <motion.button
                            ...
                            <motion.div key="idle" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center py-4 gap-2 text-white/15">
