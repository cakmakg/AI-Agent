...
import {
    ...
    RefreshCw, Database, Copy, Check, Download,
} from "lucide-react";
...
export const ArtifactViewer = () => {
    ...
    const handleDownloadPdf = () => {
        ...
        // Blueprint başlığını dosya adı olarak kullan
        const titleMatch = pendingContent.match(/^#+ (.+)/m);
        const fileName = titleMatch
            ? titleMatch[1].replace(/[^a-z0-9_-]/gi, "_").slice(0, 60)
            : "AI_Blueprint_Report";
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    h1 { font-size: 22px; color: #0a1628; border-bottom: 2px solid #0a1628; padding-bottom: 8px; margin: 32px 0 16px; }
    h2 { font-size: 16px; color: #1a3a5e; border-left: 4px solid #1a3a5e; padding-left: 10px; margin: 24px 0 12px; }
    h3 { font-size: 13px; color: #2c5282; margin: 18px 0 8px; }
    p  { margin-bottom: 10px; color: #2d3748; }
    ul, ol { margin: 8px 0 12px 20px; }
    li { margin-bottom: 4px; color: #2d3748; }
    code { font-family: 'JetBrains Mono', monospace; background: #f0f4f8; padding: 1px 5px; border-radius: 3px; font-size: 11px; }
    pre  { background: #1a202c; color: #e2e8f0; padding: 16px; border-radius: 6px; overflow-x: auto; margin: 12px 0; font-size: 11px; }
    pre code { background: none; padding: 0; color: inherit; }
    blockquote { border-left: 3px solid #4a5568; padding-left: 12px; color: #718096; margin: 10px 0; }
    hr  { border: none; border-top: 1px solid #e2e8f0; margin: 20px 0; }
    table { width: 100%; border-collapse: collapse; margin: 12px 0; }
    th { background: #edf2f7; padding: 8px 12px; text-align: left; font-size: 11px; border: 1px solid #cbd5e0; }
    td { padding: 7px 12px; border: 1px solid #e2e8f0; font-size: 11px; }
    .header { display: flex; justify-content: space-between; align-items: center;
              border-bottom: 3px solid #1a3a5e; padding-bottom: 12px; margin-bottom: 28px; }
    .badge { font-size: 9px; background: #1a3a5e; color: #fff; padding: 3px 8px; border-radius: 3px; letter-spacing: 0.1em; }
...
<body>
  <div class="header">
    <div style="font-size:18px;font-weight:700;color:#1a3a5e">AI ORCHESTRA</div>
    <span class="badge">CONFIDENTIAL — HUMAN-IN-THE-LOOP OUTPUT</span>
  </div>
  <div id="content"></div>
  <div class="footer">
    <span>Generated by AI Orchestra v2.0</span>
    <span>${new Date().toLocaleString("de-DE")}</span>
  </div>
  <script>
    document.getElementById('content').innerHTML = marked.parse(${JSON.stringify(pendingContent)});
                    ...
                    >
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <motion.button
                                ...
                                aria-label="Download artifact as PDF"
                            >
                                <Download size={9} />
                                PDF
                            </motion.button>
